data "aws_route53_zone" "default" {
  count        = module.context.enabled && length(compact(var.aliases)) > 0 ? 1 : 0
...
resource "aws_route53_record" "default" {
  for_each = module.context.enabled ? toset(compact(var.aliases)) : []
...
resource "aws_route53_record" "ipv6" {
  for_each        = module.context.enabled && var.ipv6_enabled ? toset(compact(var.aliases)) : []
  zone_id         = try(data.aws_route53_zone.default[0].zone_id, "")
  name            = each.key
  allow_overwrite = var.allow_overwrite
  type            = "AAAA"
